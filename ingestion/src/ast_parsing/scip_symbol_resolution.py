    ...
    try:
        ...
        for lang in sorted(langs):
            ...
            try:
                ...
                # No SCIP indexer is wired up for this language. Degrade
                # gracefully: warn and skip it rather than aborting the ingest.
                logger.warning("[scip] skipping %s: %s", lang, e)
                continue
            if not cmd:
                continue
            try:
                ...
                if fbk:
                    logger.warning(
                        "[scip] indexing %s failed: %s, using fallback...", lang, e
                    )
                    try:
                        _ = subprocess.run(
                            fbk,
                ...
                # The "no indexes produced at all" case is handled below so a
                # genuinely broken environment still fails loudly.
                logger.warning("[scip] skipping %s: %s", lang, e)
                continue
