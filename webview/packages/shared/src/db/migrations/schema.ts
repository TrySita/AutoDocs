...
export const contextTypeEnum = pgEnum('context_type_enum', ['file', 'page', 'none']);
...
export const tokenState = pgEnum('token_state', ['active', 'revoked', 'expired']);
...
export const anonymousUsers = pgTable(
  ...
  (table) => [unique('anonymous_users_fingerprint_unique').on(table.fingerprint)],
);
...
// Private repositories scoped to an organization
